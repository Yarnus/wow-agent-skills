...
The executable interface is tested with synthetic HTTP responses and standalone subprocess checks. Authorized live testing passed OAuth, Report Index discovery, and a participant death window, including explicit pagination termination and display truncation. Live multi-page retrieval remains unverified; synthetic tests cover it. WCL's special Environment actor ID `-1` is retained without inventing an NPC ID from `gameID=0`. Flexible raid difficulties may have empty size lists.
