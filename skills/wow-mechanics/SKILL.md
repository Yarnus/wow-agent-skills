...
2. Query a candidate signal with bounded `wcl-data events` using an explicit fight and report-relative range, or use `death-window` for a participant death review. Inspect original `abilityGameID` fields. The event command accepts fixed single-value Spell/type/participant filters, not arbitrary expressions or complete export. Missing events in a filtered range or participant window cannot establish that a mechanic did not occur elsewhere.
