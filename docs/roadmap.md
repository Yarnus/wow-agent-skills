...
`skills/wcl-data/` currently owns a standalone Python command for completed raid Boss Attempt/participant discovery, bounded attempt event queries, bounded participant death discovery, and participant death windows. Authentication, upstream queries, pagination, and Revision checks live in its single script; tests exercise the command interface with synthetic HTTP responses. No shared runtime or coaching workflow is required.

The event query accepts one optional Spell ID, one of four mechanics-oriented event types, and one Report-local attempt participant with source/target/either semantics. Filters are ANDed. Fixed Spell/type expressions narrow upstream data and are checked against returned event fields; actor filtering uses returned `sourceID` / `targetID` locally because actor expressions did not reliably match those IDs. Fractional requested bounds are applied locally after an outward-rounded upstream range. Filtered-query completeness, display truncation, whole-attempt event coverage, and Complete Bundle status are separate. The command has synthetic command coverage, copied-skill checks, and one post-implementation authorized bounded live acceptance at Revision 3: the fixed Spell/type query completed one page with 42 matches, returned three after display truncation, retained filtered rather than whole-attempt coverage, and preserved the original JSON while the observed Spell ID was localized separately. This is one narrow case, not broad report coverage. A pre-implementation authorized probe verified the same known Spell/type expression and reproduced unreliable Report-local actor expressions and fractional range over-return.

The slice has no persistent cache, export, statistics, resume, or automatic retry support. Historical authorized live testing passed OAuth, Report Index discovery (two attempts), and a fight-2 death window (687 participant events, three displayed, explicit pagination termination, unchanged Revision). A later historical Heroic Ula'tek check passed 42-page retrieval; see [live composition evidence](mechanics-live-verification.md). These records are not event-command acceptance. Regressions cover the special Environment actor `-1`, flexible raid difficulty size lists, regional report URLs, and existing credential variable names. Death discovery/window retrieval also filters returned actor IDs locally; discovery retains only attempt participants and window retrieval includes all actors only within the requested time range. Paired Revision/timestamp guards prevent stale list-to-window selection. Review follow-up rejects malformed event actor/NPC identities and verifies that the selected death appears in the complete window before publishing evidence. See [wcl-data instructions](../skills/wcl-data/SKILL.md) for the executable contract and supported source scope.
...
`skills/wow-mechanics/` provides standalone offline snapshot queries for Retail Heroic Ula'tek, with six bounded entries at build 12.1.0.69587, including three death-related effect families. Direct Heroic parent associations and unverified journal inheritance are distinguished; this is not six independent encounter mechanics. Facts, secondary/author strategies and inferred signals remain distinct. Missing version context returns structured degraded knowledge; patch-only or other-build applicability is also unknown. Explicit WCL feigns are excluded before candidate numbering; see [P1 verification](p1-acceptance-fixes.md). Optional fixed-source rechecks disclose missing content, build mismatch, transport failure and unresolved snapshot changes; they do not automatically update knowledge. No full encounter coverage, generic source framework or mechanic-rule DSL was added.

Command-level TDD regressions, copied-skill execution and synthetic mechanics → WCL death-window → localization composition pass without rewriting evidence. Live Wago row/build verification and the cited Icy Veins strategy passage check pass. A later authorized Ula'tek check passed live three-skill composition and observed three candidate spell IDs; the fourth remains unverified. Bounded attempt event querying is now available, but arbitrary expressions and complete event export remain unavailable. See [live verification](mechanics-live-verification.md). See [mechanics source notes](mechanics-source-research.md) for terms and historical coverage, [death-related source research](death-mechanics-source-research.md) for the additions, and [bounded replay](death-mechanics-verification.md) for current checks and remaining limits.
